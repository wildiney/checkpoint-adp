...
class Folha_de_Ponto:
    ...
    def adp_login(self):
        ...
        login = browser.find_element_by_xpath(
            """//*[@id="revit_form_Button_0"]""")
        login.click()
        time.sleep(5)

        self.dprint("Accessing Meu Painel")
        meuPainel = browser.find_element_by_xpath(
            """//*[@id="Meu Painel_navItem"]/span""")
        meuPainel.click()
        time.sleep(5)

        self.dprint("Accessing Ponto")
        ponto = browser.find_element_by_xpath(
            """//*[@id="painel_icones"]/span[6]/a""")
        ponto.click()
        time.sleep(5)
    ...
    def parse(self, html):
        ...
        for item in all:
            try:
                data = item.find_all('td', class_='listaval2')[
                    0].text.replace('\n', '')
            except:
                data = None
            try:
                entrada = item.find_all('td', class_='listaval2')[
                    6].find_all('font')[0].text
            except:
                entrada = None
            try:
                almoco = item.find_all('td', class_='listaval2')[
                    6].find_all('font')[4].text
            except:
                almoco = None
            try:
                retorno = item.find_all('td', class_='listaval2')[
                    6].find_all('font')[8].text
            except:
                retorno = None
            try:
                saida = item.find_all('td', class_='listaval2')[
                    6].find_all('font')[12].text
            except:
                saida = None
            ...
            if data is not None:
                ...
                if entrada is not None:
                    if entrada != ".":
                        d["lbl02-entrada"] = datetime.strptime(
                            data+entrada[-5:], '%d/%m/%Y%H:%M')
                    else:
                        d["lbl02-entrada"] = datetime.strptime(
                            data+str("00:00"), '%d/%m/%Y%H:%M')

                if almoco is not None:
                    if almoco != ".":
                        d["lbl03-almoco"] = datetime.strptime(
                            data+almoco[-5:], '%d/%m/%Y%H:%M')
                    else:
                        d["lbl03-almoco"] = datetime.strptime(
                            data+str("00:00"), '%d/%m/%Y%H:%M')
                if retorno is not None:
                    if retorno != ".":
                        d["lbl04-retorno"] = datetime.strptime(
                            data+retorno[-5:], '%d/%m/%Y%H:%M')
                    else:
                        d["lbl04-retorno"] = datetime.strptime(
                            data+str("00:00"), '%d/%m/%Y%H:%M')
                if saida is not None:
                    if saida != ".":
                        d["lbl05-saida"] = datetime.strptime(
                            data+saida[-5:], '%d/%m/%Y%H:%M')
                    else:
                        d["lbl05-saida"] = datetime.strptime(
                            data+str("00:00"), '%d/%m/%Y%H:%M')
                if data is not None:
